		       ...
		       const struct config_context *ctx, void *cb)
{
	...
	if (!strcmp(var, "diff.interhunkcontext")) {
		diff_interhunk_context_default =
			git_config_int(var, value, ctx->kvi);
...
{
	int a_width = a->indent_width, b_width = b->indent_width;
				    ...
				    const void *keydata UNUSED)
{
	...
	b = container_of(entry_or_key, const struct interned_diff_symbol, ent)
		    ->es;
...
static struct moved_entry_list *
add_lines_to_move_detection(struct diff_options *o,
		...
		if (pmb_nr == 0) {
			...
			if (!contiguous && block_length > 1)
				...
				fill_potential_moved_blocks(
					o, match, l, &pmb, &pmb_alloc, &pmb_nr);
...
static void free_diff_words_data(struct emit_callback *ecbdata)
{
	if (ecbdata->diff_words) {
		...
		free_emitted_diff_symbols(
			ecbdata->diff_words->opt->emitted_symbols);
...
static void show_stats(struct diffstat_t *data, struct diff_options *options)
{
	...
	for (i = 0; i < count; i++) {
		...
		if (file->is_binary) {
			strbuf_addf(&out, " %s%s%*s | %*s", prefix, name,
				    padding, "", number_width, "Bin");
			if (!added && !deleted) {
				strbuf_addch(&out, '\n');
		...
		strbuf_addf(&out, " %s%s%*s | %*" PRIuMAX "%s", prefix, name,
			    padding, "", number_width, added + deleted,
			    added + deleted ? " " : "");
...
static void conclude_dirstat(struct diff_options *options,
			     struct dirstat_dir *dir, unsigned long changed)
{
	struct dirstat_file *to_free = dir->files;

	if (!changed) {
		/* This can happen even with many files, if everything was
		 * renames */
		;
	} else {
...
static void checkdiff_consume_hunk(void *priv, long ob UNUSED, long on UNUSED,
...
static void add_formatted_header(struct strbuf *msg, const char *header,
				 const char *line_prefix, const char *meta,
				 const char *reset)
{
...
static int set_diff_algorithm(struct diff_options *opts, const char *alg)
{
			     ...
			     struct diff_options *o, struct diff_filepair *p)
{
	...
	if (diff_filespec_is_binary(o->repo, one) ||
		...
		if (xdi_diff_outf(&mf1, &mf2, NULL, diffstat_consume, diffstat,
				  &xpp, &xecfg))
					       ...
					       struct diff_filespec *one)
{
	...
	if (!S_ISGITLINK(one->mode) &&
		...
		if (S_ISLNK(st.st_mode)) {
			...
			prep_temp_blob(
				r->index, one->path, temp, sb.buf, sb.len,
		...
		prep_temp_blob(r->index, one->path, temp, one->data, one->size,
			       &one->oid, one->mode);
	}
	return temp;
}
			  ...
			  int use_color)
{
	...
	*must_show_header = 1;
	...
	if ((more_headers = additional_headers(o, name))) {
		add_formatted_headers(msg, more_headers, line_prefix, set,
				      reset);
		*must_show_header = 1;
	}
			 ...
			 struct diffstat_t *diffstat)
{
	...
	if (!o->ignore_driver_algorithm) {
		struct userdiff_driver *drv =
			userdiff_find_by_path(o->repo->index, p->one->path);
...
int diff_check_follow_pathspec(struct pathspec *ps, int die_on_error)
{
	...
	forbidden_magic = ps->items[0].magic &
...
void diff_setup_done(struct diff_options *options)
{
	...
	if (HAS_MULTI_BITS(options->pickaxe_opts &
		...
		die(_("options '%s' and '%s' cannot be used together, use '%s' with '%s' and '%s'"),
		    "--pickaxe-all", "--find-object", "--pickaxe-all", "-G",
		    "-S");
...
static int diff_opt_default_prefix(const struct option *opt, const char *optarg,
				   int unset)
{
	struct diff_options *options = opt->value;
		   ...
		   const char *prefix)
{
	...
	ac = parse_options(
		ac, av, prefix, parseopts, NULL,
		PARSE_OPT_KEEP_DASHDASH | PARSE_OPT_KEEP_UNKNOWN_OPT |
			PARSE_OPT_NO_INTERNAL_HELP | PARSE_OPT_ONE_SHOT |
...
static void diff_flush_patch(struct diff_filepair *p, struct diff_options *o)
{
	int include_conflict_headers =
		(additional_headers(o, p->one->path) && !o->pickaxe_opts &&
			     ...
			     struct object_id *oid, int diff_header_only)
{
	...
	for (i = 0; i < q->nr; i++) {
		...
		if (diff_header_only) {
			/* don't do anything since we're only populating header
			 * info */
			...
			if (p->one->mode == 0) {
				...
				patch_id_add_string(&ctx, "+++b/");
				the_hash_algo->update_fn(&ctx, p->two->path,
							 len2);
			} else if (p->two->mode == 0) {
				patch_id_add_string(&ctx, "---a/");
				the_hash_algo->update_fn(&ctx, p->one->path,
							 len1);
				patch_id_add_string(&ctx, "+++/dev/null");
			} else {
				patch_id_add_string(&ctx, "---a/");
...
static void
create_filepairs_for_header_only_notifications(struct diff_options *o)
{
...
static void diff_flush_patch_all_file_pairs(struct diff_options *o)
{
	...
	if (o->emitted_symbols) {
		if (o->color_moved) {
			...
			entry_list =
				add_lines_to_move_detection(o, &entry_pool);
