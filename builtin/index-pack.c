...
static const char *open_pack_file(const char *pack_name)
{
	if (from_stdin) {
		...
		if (!pack_name) {
			...
			output_fd = xopen(pack_name, O_CREAT | O_EXCL | O_RDWR,
					  0600);
		}
			...
			const struct object_id *oid)
{
	...
	if (startup_info->have_repository) {
		read_lock();
		collision_test_needed = repo_has_object_file_with_flags(
	...
	if (collision_test_needed) {
		...
		if (has_type != type || has_size != size)
			die(_("SHA1 COLLISION FOUND WITH %s !"),
			    oid_to_hex(oid));
		has_data = repo_read_object_file(the_repository, oid, &has_type,
						 &has_size);
		read_unlock();
		if (!data)
...
static void parse_pack_objects(unsigned char *hash)
{
	...
	if (verbose)
		progress = start_progress(progress_title ? progress_title :
...
static void rename_tmp_packfile(const char **final_name, const char *curr_name,
		  ...
		  const char *promisor_msg, unsigned char *hash)
{
	...
	if (!from_stdin) {
		...
		fsync_component_or_die(FSYNC_COMPONENT_PACK, output_fd,
				       curr_pack_name);
		if (close(output_fd))
	...
	rename_tmp_packfile(&final_pack_name, curr_pack_name, &pack_name, hash,
			    "pack", from_stdin);
...
int cmd_index_pack(int argc, const char **argv, const char *prefix)
{
	...
	if (fix_thin_pack && !from_stdin)
		die(_("the option '%s' requires '%s'"), "--fix-thin",
		    "--stdin");
