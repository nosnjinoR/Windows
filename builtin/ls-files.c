			...
			const char *format, const char *fullname)
{
	...
	while (strbuf_expand_step(&sb, &format)) {
		...
		if (skip_prefix(format, "%", &format))
			...
			strbuf_addstr(&sb,
				      S_ISREG(ce->ce_mode) ?
					      get_cached_convert_stats_ascii(
						      repo->index, ce->name) :
					      "");
		else if (skip_prefix(format, "(eolinfo:worktree)", &format))
			strbuf_addstr(
				&sb,
				!lstat(fullname, &st) && S_ISREG(st.st_mode) ?
...
int cmd_ls_files(int argc, const char **argv, const char *cmd_prefix)
{
	...
	if (format &&
	    (show_stage || show_others || show_killed || show_resolve_undo ||
	     skipping_duplicates || show_eol || show_tag))
	...
	if (recurse_submodules &&
	    (show_deleted || show_others || show_unmerged || show_killed ||
	     show_modified || show_resolve_undo || with_tree))
