...
static void show_rev(int type, const struct object_id *oid, const char *name)
{
	...
	if ((symbolic || abbrev_ref) && name) {
		...
		show_with_type(type, repo_find_unique_abbrev(the_repository,
							     oid, abbrev));
	else
...
static int try_difference(const char *arg)
{
	...
	*dotdot = 0;
	...
	if (!repo_get_oid_committish(the_repository, start, &start_oid) &&
	    !repo_get_oid_committish(the_repository, end, &end_oid)) {
		...
		if (symmetric) {
			...
			if (repo_get_merge_bases(the_repository, a, b,
						 &exclude) < 0)
				exit(128);
			while (exclude) {
		...
		if (!seen_end_of_options && *arg == '-') {
			...
			if (opt_with_value(arg, "--path-format", &arg)) {
				...
				if (!strcmp(arg, "absolute")) {
					...
					die(_("unknown argument to --path-format: %s"),
					    arg);
				}
				continue;
			}
			...
			if (opt_with_value(arg, "--output-object-format",
					   &arg)) {
				if (!arg)
