...
static void status_init_config(struct wt_status *s, config_fn_t fn)
{
	...
	git_config(fn, s);
	determine_whence(s);
	s->hints = advice_enabled(ADVICE_STATUS_HINTS); /* must come after
							   git_config() */
}
...
static void refresh_cache_or_die(int refresh_flags)
{
	 ...
	 */
	if (refresh_index(&the_index, refresh_flags | REFRESH_IN_PORCELAIN,
			  NULL, NULL, NULL))
	...
	if (pathspec_from_file) {
		if (interactive)
			die(_("options '%s' and '%s' cannot be used together"),
			    "--pathspec-from-file", "--interactive/--patch");

		if (all)
			die(_("options '%s' and '%s' cannot be used together"),
			    "--pathspec-from-file", "-a");
		      ...
		      int nowarn, struct wt_status *s)
{
	...
	s->fp = fp;
	s->nowarn = nowarn;
	s->is_initial = repo_get_oid(the_repository, s->reference, &oid) ? 1 :
									   0;
	if (!s->is_initial)
			     ...
			     struct strbuf *author_ident)
{
	...
	if (signoff)
		append_signoff(&sb, ignored_log_message_bytes(sb.buf, sb.len),
			       0);
	...
	if (use_editor && include_status) {
		...
		if (cleanup_mode == COMMIT_MSG_CLEANUP_ALL)
			status_printf(s, GIT_COLOR_NORMAL, hint_cleanup_all,
				      comment_line_str);
		else if (cleanup_mode == COMMIT_MSG_CLEANUP_SCISSORS) {
			if (whence == FROM_COMMIT)
				wt_status_add_cut_line(s);
		} else /* COMMIT_MSG_CLEANUP_SPACE, that is. */
			status_printf(s, GIT_COLOR_NORMAL, hint_cleanup_space,
				      comment_line_str);
...
static void check_fixup_reword_options(int argc, const char *argv[])
{
	...
	if (argc)
		die(_("reword option of '%s' and path '%s' cannot be used together"),
		    "--fixup", *argv);
				      ...
				      struct wt_status *s)
{
	...
	if (use_message || edit_message || logfile || fixup_message ||
	    have_option_m)
		template_file = NULL;
	if (edit_message)
	...
	die_for_incompatible_opt4(also, "-i/--include", only, "-o/--only", all,
				  "-a/--all", interactive,
				  "--interactive/-p/--patch");
	if (fixup_message) {
		/*
		...
		if (p > fixup_message && *p == ':') {
			*p = '\0';
			...
			if (!strcmp("amend", fixup_message) ||
				...
				die(_("unknown option: --fixup=%s:%s"),
				    fixup_message, fixup_commit);
			}
		} else {
			     ...
			     const struct config_context *ctx, void *cb)
{
	...
	if (!strcmp(k, "status.submodulesummary")) {
		int is_bool;
		s->submodule_summary =
			git_config_bool_or_int(k, v, ctx->kvi, &is_bool);
...
int cmd_commit(int argc, const char **argv, const char *prefix)
{
	...
	static struct option builtin_commit_options[] = {
		...
		OPT_STRING(
			...
			N_("[(amend|reword):]commit"),
			N_("use autosquash formatted message to fixup or amend/reword specified commit")),
		OPT_STRING(
