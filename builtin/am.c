...
enum empty_action {
	STOP_ON_EMPTY_COMMIT = 0, /* output errors and stop in the middle of an
				     am session */
	DROP_EMPTY_COMMIT, /* skip with a notice message, unless "--quiet" has
			      been passed */
...
static int am_option_parse_quoted_cr(const struct option *opt, const char *arg,
				     int unset)
{
...
static int am_option_parse_empty(const struct option *opt, const char *arg,
				 int unset)
{
...
say(const struct am_state *state, FILE *fp, const char *fmt, ...)
{
	va_list ap;
...
static void am_load(struct am_state *state)
{
	...
	if (!*sb.buf)
		state->quoted_cr = quoted_cr_unset;
	else if (mailinfo_parse_quoted_cr_action(sb.buf, &state->quoted_cr) !=
		 0)
...
static int run_applypatch_msg_hook(struct am_state *state)
{
	...
	if (!state->no_verify)
		ret = run_hooks_l("applypatch-msg",
...
static void NORETURN die_user_resolve(const struct am_state *state)
{
	if (state->resolvemsg) {
		...
		if (advice_enabled(ADVICE_AM_WORK_DIR) &&
			...
			strbuf_addf(
				&sb,
				_("To record the empty patch as an empty commit, run \"%s --allow-empty\".\n"),
				cmdline);
...
static int run_apply(const struct am_state *state, const char *index_file)
{
	...
	opts_left = apply_parse_options(apply_opts.nr, apply_argv, &apply_state,
					&force_apply, &options, NULL);
 ...
 * trust and commit what the user has in the index and working tree. If
 * `allow_empty` is true, commit as an empty commit when index has not changed
 * and lacking a patch.
 */
...
static int fast_forward_to(struct tree *head, struct tree *remote, int reset)
{
	...
	init_tree_desc(&t[1], &remote->object.oid, remote->buffer,
		       remote->size);
...
int cmd_am(int argc, const char **argv, const char *prefix)
{
	...
	struct option options[] = {
		...
		{ OPTION_STRING, 'S', "gpg-sign", &state.sign_commit,
		  ...
		  (intptr_t) "" },
		OPT_CALLBACK_F(0, "empty", &state.empty_type,
			       "(stop|drop|keep)",
	...
	if (in_progress) {
		...
		if (file_exists(state.dir) && !state.rebasing) {
			if (resume_mode == RESUME_ABORT ||
			    resume_mode == RESUME_QUIT) {
				am_destroy(&state);
