...
static int set_diff_algorithm(xpparam_t *xpp, const char *alg)
{
...
static int diff_algorithm_cb(const struct option *opt, const char *arg,
			     int unset)
{
...
int cmd_merge_file(int argc, const char **argv, const char *prefix)
{
	...
	for (i = 0; i < 3; i++) {
		...
		if (ret != -1 && (mmf->size > MAX_XDIFF_SIZE ||
			...
			ret = error("Cannot merge binary files: %s", argv[i]);
		}
	...
	if (ret >= 0) {
		if (object_id && !to_stdout) {
			struct object_id oid;
			if (result.size) {
				if (write_object_file(result.ptr, result.size,
						      OBJ_BLOB, &oid) < 0)
					ret = error(_(
			...
			if (!f)
				...
				ret = error_errno("Could not write to %s",
						  filename);
			else if (fclose(f))
				ret = error_errno("Could not close %s",
						  filename);
			free(fpath);
		}
		free(result.ptr);
