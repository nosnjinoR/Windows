	...
	case 0: {
		...
		if (exp_type_id == OBJ_BLOB) {
			...
			if (oid_object_info(the_repository, &oid, NULL) ==
			    OBJ_TAG) {
				char *buffer = repo_read_object_file(
					the_repository, &oid, &type, &size);
				const char *target;
			...
			if (opt->transform_mode == 'w') {
				...
				if (!textconv_object(the_repository, data->rest,
					...
					contents = repo_read_object_file(
						the_repository, oid, &type,
						&size);
				if (!contents)
					die("could not convert '%s' %s",
					    oid_to_hex(oid), data->rest);
			} else
				BUG("invalid transform_mode: %c",
				    opt->transform_mode);
...
static void print_default_format(struct strbuf *scratch,
				 struct expand_data *data,
				 struct batch_options *opt)
{
	strbuf_addf(scratch, "%s %s %" PRIuMAX "%c", oid_to_hex(&data->oid),
		    type_name(data->type), (uintmax_t)data->size,
		    opt->output_delim);
}
	...
	if (!data->skip_object_info) {
		...
		if (pack)
			...
			ret = oid_object_info_extended(
				the_repository, &data->oid, &data->info,
		...
		if (use_mailmap &&
			...
			buf = repo_read_object_file(the_repository, &data->oid,
						    &data->type, &data->size);
			if (!buf)
				die(_("unable to read %s"),
				    oid_to_hex(&data->oid));
			     ...
			     struct expand_data *data)
{
	...
	if (result != FOUND) {
		...
		case DANGLING_SYMLINK:
			printf("dangling %" PRIuMAX "%c%s%c",
			       (uintmax_t)strlen(obj_name), opt->output_delim,
			       obj_name, opt->output_delim);
			break;
		case SYMLINK_LOOP:
	...
	if (ctx.mode == 0) {
		printf("symlink %" PRIuMAX "%c%s%c",
		       (uintmax_t)ctx.symlink_path.len, opt->output_delim,
		       ctx.symlink_path.buf, opt->output_delim);
		fflush(stdout);
		return;
	}
...
static int batch_object_cb(const struct object_id *oid, void *vdata)
{
	...
	batch_object_write(NULL, data->scratch, data->opt, data->expand, NULL,
			   0);
	return 0;
}
				  ...
				  void *vdata)
{
	...
	oidcpy(&data->expand->oid, oid);
	batch_object_write(NULL, data->scratch, data->opt, data->expand, pack,
			   offset);
	return 0;
}
...
static int batch_unordered_packed(const struct object_id *oid,
				  ...
				  void *data)
{
	return batch_unordered_object(
		oid, pack, nth_packed_object_offset(pack, pos), data);
}
...
static void parse_cmd_contents(struct batch_options *opt, const char *line,
			       struct strbuf *output, struct expand_data *data)
{
	opt->batch_mode = BATCH_MODE_CONTENTS;
	batch_one_object(line, output, opt, data);
}

static void parse_cmd_info(struct batch_options *opt, const char *line,
			   struct strbuf *output, struct expand_data *data)
{
...
static void dispatch_calls(struct batch_options *opt, struct strbuf *output,
			   struct expand_data *data, struct queued_cmd *cmd,
			   int nr)
{
	int i;
...
static int batch_objects(struct batch_options *opt)
{
	...
	data.mark_query = 1;
	expand_format(&output, opt->format ? opt->format : DEFAULT_FORMAT,
		      &data);
	data.mark_query = 0;
...
int cmd_cat_file(int argc, const char **argv, const char *prefix)
{
	...
	const struct option options[] = {
		...
		OPT_CMDMODE('e', NULL, &opt, N_("check if <object> exists"),
			    'e'),
		...
		OPT_CMDMODE(
			't', NULL, &opt,
			N_("show object type (one of 'blob', 'tree', 'commit', 'tag', ...)"),
			't'),
		OPT_CMDMODE('s', NULL, &opt, N_("show object size"), 's'),
		OPT_BOOL(
			0, "allow-unknown-type", &unknown_type,
			N_("allow -s and -t to work with broken/corrupt objects")),
		OPT_BOOL(0, "use-mailmap", &use_mailmap,
		...
		OPT_CALLBACK_F(0, "batch-check", &batch, N_("format"),
			       ...
			       PARSE_OPT_OPTARG | PARSE_OPT_NONEG,
			       batch_option_callback),
		OPT_BOOL_F('z', NULL, &input_nul_terminated,
			   N_("stdin is NUL-terminated"), PARSE_OPT_HIDDEN),
		OPT_BOOL('Z', NULL, &nul_terminated,
		...
		OPT_STRING(
			0, "path", &force_path, N_("blob|tree"),
	...
	if (force_path && !opt_cw)
		usage_msg_optf(_("'%s=<%s>' needs '%s' or '%s'"), usage,
			       options, "--path", _("path|tree-ish"),
			       "--filters", "--textconv");
	...
	if (batch.enabled) {
		if (opt_cw)
			...
			usage_msg_optf(
				_("'-%c' is incompatible with batch mode"),
				usage, options, opt);
		else if (argc)
	...
	if (opt) {
		if (!argc && opt == 'c')
			usage_msg_optf(_("<rev> required with '%s'"), usage,
				       options, "--textconv");
		else if (!argc && opt == 'w')
			usage_msg_optf(_("<rev> required with '%s'"), usage,
				       options, "--filters");
		else if (!argc && opt_epts)
			usage_msg_optf(_("<object> required with '-%c'"), usage,
				       options, opt);
		else if (argc == 1)
			obj_name = argv[0];
		else
		...
		usage_msg_optf(
			_("only two arguments allowed in <type> <object> mode, not %d"),
			usage, options, argc);
	} else if (argc) {
		exp_type = argv[0];
		obj_name = argv[1];
