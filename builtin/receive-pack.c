			       ...
			       const struct config_context *ctx, void *cb)
{
	const char *msg_id;
	int status =
		parse_hide_refs_config(var, value, "receive", &hidden_refs);
...
static void prepare_push_cert_sha1(struct child_process *proc)
{
	...
	if (!is_null_oid(&push_cert_oid)) {
		...
		strvec_pushf(&proc->env, "GIT_PUSH_CERT_STATUS=%c",
			     sigcheck.result);
		if (push_cert_nonce) {
			strvec_pushf(&proc->env, "GIT_PUSH_CERT_NONCE=%s",
				     push_cert_nonce);
...
static const char *push_to_checkout(unsigned char *hash, int *invoked_hook,
				    struct strvec *env, const char *work_tree)
{
...
static const char *update(struct command *cmd, struct shallow_info *si)
{
	...
	if (!is_null_oid(new_oid) &&
	    !repo_has_object_file(the_repository, new_oid)) {
	...
	if (deny_non_fast_forwards && !is_null_oid(new_oid) &&
		...
		ret2 = repo_in_merge_bases(the_repository, old_commit,
					   new_commit);
		if (ret2 < 0)
			exit(128);
		if (!ret2) {
					  ...
					  const char *dst_name, int flag)
{
	...
	rp_error("refusing inconsistent update between symref '%s' (%s..%s) and"
		 " its target '%s' (%s..%s)",
		 cmd->ref_name,
		 repo_find_unique_abbrev(the_repository, &cmd->old_oid,
					 DEFAULT_ABBREV),
		 repo_find_unique_abbrev(the_repository, &cmd->new_oid,
					 DEFAULT_ABBREV),
		 dst_cmd->ref_name,
		 repo_find_unique_abbrev(the_repository, &dst_cmd->old_oid,
					 DEFAULT_ABBREV),
		 repo_find_unique_abbrev(the_repository, &dst_cmd->new_oid,
					 DEFAULT_ABBREV));
			     ...
			     const struct string_list *push_options)
{
	 ...
	 * destroy temporary data in the quarantine area.
	 */
...
static const char *unpack(int err_fd, struct shallow_info *si)
{
	...
	int status;
	struct child_process child = CHILD_PROCESS_INIT;
	int fsck_objects = (receive_fsck_objects >= 0  ? receive_fsck_objects :
