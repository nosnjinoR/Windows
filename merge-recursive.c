...
__attribute__((format(printf, 2, 3))) static int err(struct merge_options *opt,
						     const char *err, ...)
{
	va_list params;
				     ...
				     struct commit *commit)
{
	...
	if (desc)
		...
		strbuf_repo_add_unique_abbrev(
			&opt->obuf, repo, &commit->object.oid, DEFAULT_ABBREV);
		strbuf_addch(&opt->obuf, ' ');
		if (repo_parse_commit(repo, commit) != 0)
			...
			const char *title;
			const char *msg =
				repo_get_commit_buffer(repo, commit, NULL);
...
static void get_files_dirs(struct merge_options *opt, struct tree *tree)
{
	...
	read_tree(opt->repo, tree, &match_all, save_files_dirs, opt);
}
		      ...
		      const char *branch2, const int extra_marker_size)
{
	...
	if (merge_status == LL_MERGE_BINARY_CONFLICT)
		warning("Cannot merge binary files: %s (%s vs. %s)", a->path,
			name1, name2);
			   ...
			   const struct object_id *a, const struct object_id *b)
{
	...
	if (ret2 < 0) {
		output(opt, 1,
		       _("Failed to merge submodule %s (repository corrupt)"),
		       path);
		ret = -1;
		goto cleanup;
	}
	...
	parent_count =
		find_first_merges(&subrepo, &merges, path, commit_a, commit_b);
	switch (parent_count) {
	case -1:
		output(opt, 1,
		       _("Failed to merge submodule %s (repository corrupt)"),
				...
				struct rename_conflict_info *ci)
{
	...
	if (clean &&
		...
		if (update_stages(opt, path, &mfi.blob, &mfi.blob, &mfi.blob))
			return -1;
