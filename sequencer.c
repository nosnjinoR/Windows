...
static GIT_PATH_FUNC(git_path_seq_dir, "sequencer")
	...
	/*
	 * The update-refs file stores a list of refs that will be updated at
	 * the end of the rebase sequence. The 'update-ref <ref>' commands in
	 * the todo file update the OIDs for the refs in this file, but the refs
	 * are not updated until the end of the rebase sequence.
	 *
	 * rebase_path_update_refs() returns the path to this file for a given
	 * worktree directory. For the current worktree, pass
...
static GIT_PATH_FUNC(rebase_path_gpg_sign_opt, "rebase-merge/gpg_sign_opt") static GIT_PATH_FUNC(rebase_path_cdate_is_adate, "rebase-merge/cdate_is_adate") static GIT_PATH_FUNC(rebase_path_ignore_date, "rebase-merge/ignore_date") static GIT_PATH_FUNC(rebase_path_orig_head, "rebase-merge/orig-head") static GIT_PATH_FUNC(rebase_path_verbose, "rebase-merge/verbose") static GIT_PATH_FUNC(rebase_path_quiet, "rebase-merge/quiet") static GIT_PATH_FUNC(
																							     ...
																							     "rebase-merge/strategy") static GIT_PATH_FUNC(rebase_path_strategy_opts,
																													   "rebase-merge/strategy_opts") static GIT_PATH_FUNC(rebase_path_allow_rerere_autoupdate,
																																			      "rebase-merge/allow_rerere_autoupdate") static GIT_PATH_FUNC(rebase_path_reschedule_failed_exec,
																																											   "rebase-merge/reschedule-failed-exec") static GIT_PATH_FUNC(rebase_path_no_reschedule_failed_exec,
																																																		       "rebase-merge/no-reschedule-failed-exec") static GIT_PATH_FUNC(rebase_path_drop_redundant_commits,
	 ...
	 * list. We use a string_list to map refs to these (before, after)
	 * pairs.
	 */
...
static const char *short_commit_name(struct repository *r,
...
const char *rebase_resolvemsg = N_(
...
static int is_index_unchanged(struct repository *r)
{
	...
	if (!resolve_ref_unsafe("HEAD", RESOLVE_REF_READING, &head_oid, NULL)) {
		...
		head_name = resolve_ref_unsafe(
			"HEAD", RESOLVE_REF_READING | RESOLVE_REF_NO_RECURSE,
			&head_oid, NULL);
		if (!head_name || !starts_with(head_name, "refs/heads/") ||
		 ...
		 * a commit object already.  repo_parse_commit() will return
		 * failure without further complaints in such a case. Otherwise,
		 * if the commit is invalid, repo_parse_commit() will complain.
		 * So there is nothing for us to say here.  Just return failure.
		 */
			 ...
			 unsigned int flags, struct object_id *oid)
{
	...
	if (flags & AMEND_MSG) {
		...
		const char *message =
			repo_logmsg_reencode(r, current_head, NULL, out_enc);
				  ...
				  struct replay_opts *opts, unsigned flag)
{
	...
	if (opts->current_fixup_count > 0) {
		...
		if (command == TODO_FIXUP && !flag &&
		    write_message(body, strlen(body), rebase_path_fixup_msg(),
				  0) < 0) {
...
static void refer_to_commit(struct replay_opts *opts, struct strbuf *msgbuf,
			    struct commit *commit)
{
...
static int do_pick_commit(struct repository *r, struct todo_item *item,
			  ...
			  int *check_todo)
{
		...
		if (opts->commit_use_reference) {
			strbuf_addstr(
				...
				"# *** SAY WHY WE ARE REVERTING ON THE TITLE LINE ***");
				       ...
				       &orig_subject) &&
			   /*
			    * We don't touch pre-existing repeated reverts,
			    * because theoretically these can be nested
			    * arbitrarily deeply, thus requiring excessive
	...
	if (is_rebase_i(opts) && write_author_script(msg.message) < 0)
		...
		res |= try_merge_command(r, opts->strategy, opts->xopts.nr,
					 opts->xopts.v, common,
			   ...
			   const char *buf, const char *bol, char *eol)
{
	...
	if (bol == eol || *bol == '\r' ||
	    starts_with_mem(bol, eol - bol, comment_line_str)) {
			    ...
			    const struct config_context *ctx, void *data)
{
	...
	if (!value)
		...
		opts->no_commit = git_config_bool_or_int(key, value, ctx->kvi,
							 &error_flag);
	else if (!strcmp(key, "options.edit"))
		opts->edit = git_config_bool_or_int(key, value, ctx->kvi,
						    &error_flag);
	else if (!strcmp(key, "options.allow-empty"))
		opts->allow_empty = git_config_bool_or_int(key, value, ctx->kvi,
							   &error_flag);
	else if (!strcmp(key, "options.allow-empty-message"))
		opts->allow_empty_message = git_config_bool_or_int(
			key, value, ctx->kvi, &error_flag);
	else if (!strcmp(key, "options.drop-redundant-commits"))
		opts->drop_redundant_commits = git_config_bool_or_int(
			key, value, ctx->kvi, &error_flag);
	else if (!strcmp(key, "options.keep-redundant-commits"))
		opts->keep_redundant_commits = git_config_bool_or_int(
			key, value, ctx->kvi, &error_flag);
	else if (!strcmp(key, "options.signoff"))
		opts->signoff = git_config_bool_or_int(key, value, ctx->kvi,
						       &error_flag);
	else if (!strcmp(key, "options.record-origin"))
		opts->record_origin = git_config_bool_or_int(
			key, value, ctx->kvi, &error_flag);
	else if (!strcmp(key, "options.allow-ff"))
		opts->allow_ff = git_config_bool_or_int(key, value, ctx->kvi,
							&error_flag);
...
static int read_populate_opts(struct replay_opts *opts)
{
	if (is_rebase_i(opts)) {
		...
		if (read_oneliner(&buf, rebase_path_squash_onto(), 0)) {
			if (repo_get_oid_committish(the_repository, buf.buf,
						    &opts->squash_onto) < 0) {
				   ...
				   struct replay_opts *opts)
{
	...
	while ((commit = get_revision(opts->revs))) {
		...
		const char *commit_buffer = repo_logmsg_reencode(
			the_repository, commit, NULL, encoding);
		const char *subject;
		int subject_len;
...
static int save_opts(struct replay_opts *opts)
{
	...
	if (opts->no_commit)
		res |= git_config_set_in_file_gently(
			opts_file, "options.no-commit", NULL, "true");
	if (opts->edit >= 0)
		res |= git_config_set_in_file_gently(
			opts_file, "options.edit", NULL,
			opts->edit ? "true" : "false");
	if (opts->allow_empty)
		res |= git_config_set_in_file_gently(
			opts_file, "options.allow-empty", NULL, "true");
	if (opts->allow_empty_message)
		res |= git_config_set_in_file_gently(
			opts_file, "options.allow-empty-message", NULL, "true");
	if (opts->drop_redundant_commits)
		res |= git_config_set_in_file_gently(
			opts_file, "options.drop-redundant-commits", NULL,
			"true");
	if (opts->keep_redundant_commits)
		res |= git_config_set_in_file_gently(
			opts_file, "options.keep-redundant-commits", NULL,
			"true");
	if (opts->signoff)
		res |= git_config_set_in_file_gently(
			opts_file, "options.signoff", NULL, "true");
	if (opts->record_origin)
		res |= git_config_set_in_file_gently(
			opts_file, "options.record-origin", NULL, "true");
	if (opts->allow_ff)
		res |= git_config_set_in_file_gently(
			opts_file, "options.allow-ff", NULL, "true");
	if (opts->mainline) {
		struct strbuf buf = STRBUF_INIT;
		strbuf_addf(&buf, "%d", opts->mainline);
		res |= git_config_set_in_file_gently(
			opts_file, "options.mainline", NULL, buf.buf);
		strbuf_release(&buf);
	}
	if (opts->strategy)
		res |= git_config_set_in_file_gently(
			opts_file, "options.strategy", NULL, opts->strategy);
	if (opts->gpg_sign)
		res |= git_config_set_in_file_gently(
			opts_file, "options.gpg-sign", NULL, opts->gpg_sign);
	for (size_t i = 0; i < opts->xopts.nr; i++)
		res |= git_config_set_multivar_in_file_gently(
			opts_file, "options.strategy-option", opts->xopts.v[i],
			"^$", NULL, 0);
	if (opts->allow_rerere_auto)
		res |= git_config_set_in_file_gently(
			opts_file, "options.allow-rerere-auto", NULL,
			opts->allow_rerere_auto == RERERE_AUTOUPDATE ? "true" :
								       "false");

	if (opts->explicit_cleanup)
		res |= git_config_set_in_file_gently(
			opts_file, "options.default-msg-cleanup", NULL,
		      ...
		      struct replay_opts *opts)
{
	...
	if (!file_exists(rebase_path_message())) {
		...
		const char *commit_buffer =
			repo_logmsg_reencode(r, commit, NULL, encoding);
		find_commit_subject(commit_buffer, &subject);
		res |= write_message(subject, strlen(subject),
	...
	if (to_amend) {
		...
		if (commit)
			fprintf_ln(stderr, _("Could not apply %s... %.*s"),
				   short_commit_name(r, commit), subject_len,
				   subject);
		else
			/*
...
safe_append(const char *filename, const char *fmt, ...)
{
	va_list ap;
...
static const char *sequencer_reflog_action(struct replay_opts *opts)
{
	if (!opts->reflog_action) {
		...
		opts->reflog_action = xstrdup(opts->reflog_action ?
						      opts->reflog_action :
						      action_name(opts));
	}
...
__attribute__((format(printf, 3, 4))) static const char *
reflog_message(struct replay_opts *opts, const char *sub_action,
	       const char *fmt, ...)
{
	va_list ap;
	static struct strbuf buf = STRBUF_INIT;
...
static int do_merge(struct repository *r, struct commit *commit,
		    const char *arg, int arg_len, int flags, int *check_todo,
		    struct replay_opts *opts)
{
	...
	if (!opts->strategy || !strcmp(opts->strategy, "ort")) {
		 ...
		 */
		ret = merge_ort_recursive(&o, head_commit, merge_commit, bases,
					  &i);
	} else {
		ret = merge_recursive(&o, head_commit, merge_commit, bases, &i);
	}
	if (ret <= 0)
		fputs(o.obuf.buf, stdout);
	strbuf_release(&o.obuf);
...
static int write_update_refs_state(struct string_list *refs_to_oids)
{
	...
	if (hold_lock_file_for_update(&lock, path, 0) < 0) {
		result = error(
			_("another 'rebase' process appears to be running; "
				  ...
				  struct todo_list *todo_list)
{
	...
	for (i = 0; i < update_refs.nr; i++) {
		...
		if (!found) {
			...
			update_refs.nr--;
			MOVE_ARRAY(update_refs.items + i,
				   update_refs.items + i + 1,
				   update_refs.nr - i);
...
static int do_update_refs(struct repository *r, int quiet)
{
	...
	if (!quiet && (update_msg.len || error_msg.len)) {
		fprintf(stderr, _("Updated the following refs with %s:\n%s"),
			"--update-refs", update_msg.buf);

		if (res)
			fprintf(stderr,
				_("Failed to update the following refs with %s:\n%s"),
				"--update-refs", error_msg.buf);
	}
...
static void create_autostash_internal(struct repository *r, const char *path,
				      const char *refname)
{
	struct strbuf buf = STRBUF_INIT;
				    ...
				    int attempt_apply)
{
	...
	refs_delete_ref(get_main_ref_store(r), "", refname, &stash_oid,
			REF_NO_DEREF);
...
static int pick_one_commit(struct repository *r, struct todo_list *todo_list,
			   struct replay_opts *opts, int *check_todo,
			   int *reschedule)
{
	int res;
	...
	if (item->command == TODO_EDIT) {
		...
		if (!res) {
			...
			fprintf(stderr, _("Stopped at %s...  %.*s\n"),
				short_commit_name(r, commit), item->arg_len,
				arg);
		}
	...
	if (res && is_fixup(item->command)) {
		if (res == 1)
			intend_to_amend();
		return error_failed_squash(r, item->commit, opts, item->arg_len,
					   arg);
		...
		return res | error_with_patch(r, item->commit, arg,
	...
	if (file_exists(rebase_path_amend())) {
		...
		if (!is_clean || !opts->current_fixup_count)
			...
			if (opts->current_fixup_count > 0 &&
				...
				p = repo_logmsg_reencode(r, commit, NULL,
							 encoding);
				if (!p) {
					res = error(
						_("could not parse commit %s"),
						oid_to_hex(
							&commit->object.oid));
				   ...
				   struct todo_add_branch_context *ctx)
{
	...
	const char *head_ref =
		resolve_ref_unsafe("HEAD", RESOLVE_REF_READING, NULL, NULL);
		    ...
		    unsigned update_refs, struct todo_list *todo_list)
{
	...
	repo_find_unique_abbrev_r(r, shortonto, &oid, DEFAULT_ABBREV);
...
int todo_list_rearrange_squash(struct todo_list *todo_list)
{
	...
	for (i = 0; i < todo_list->nr; i++) {
		...
		commit_buffer = repo_logmsg_reencode(
			the_repository, item->commit, NULL, "UTF-8");
		find_commit_subject(commit_buffer, &subject);
				    ...
				    struct string_list *refs)
{
	...
	while (strbuf_getline(&ref, fp) != EOF) {
		...
		if (strbuf_getline(&hash, fp) == EOF ||
			...
			warning(_("update-refs file at '%s' is invalid"), path);
			result = -1;
			goto cleanup;
		}
