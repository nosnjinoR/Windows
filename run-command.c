...
static void child_err_spew(struct child_process *cmd, struct child_err *cerr)
{
	...
	case CHILD_ERR_CHDIR:
		error_errno("exec '%s': cd to '%s' failed", cmd->args.v[0],
			    cmd->dir);
		break;
	case CHILD_ERR_DUP2:
	...
	if (need_err) {
		...
		fail_pipe:
			error("cannot create %s pipe for %s: %s", str,
			      cmd->args.v[0], strerror(failed_errno));
...
static void
kill_children_signal(const struct parallel_processes_for_signal *pp_sig,
		     int signo)
{
		    ...
		    struct parallel_processes_for_signal *pp_sig)
{
	...
	trace_printf("run_processes_parallel: preparing to run up to %" PRIuMAX
		     " tasks",
		     (uintmax_t)n);
