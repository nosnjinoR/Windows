			       ...
			       const char *name, int namelen, unsigned flags)
{
	...
	if (item->attr_match_nr &&
	    !match_pathspec_attrs(istate, name - prefix, namelen + prefix,
				  item))
		return 0;
...
int match_pathspec(struct index_state *istate, const struct pathspec *ps,
...
int pl_hashmap_cmp(const void *cmp_data UNUSED, const struct hashmap_entry *a,
		   const struct hashmap_entry *b, const void *key UNUSED)
{
				    ...
				    struct path_pattern *given)
{
	...
	hashmap_entry_init(&translated->ent, fspathhash(translated->pattern));
...
{
	dir->internal.unmanaged_exclude_files++; /* see
				     ...
				     int require_cone_mode)
{
	 ...
	 */
	if (!*path || init_sparse_checkout_patterns(istate) ||
	    (require_cone_mode &&
	...
	for (end = path + strlen(path); end > path && match == UNDECIDED;
		...
		match = path_matches_pattern_list(
			...
			&dtype, istate->sparse_checkout_patterns, istate);
			 ...
			 const char *base, int baselen)
{
	...
	while (current < baselen) {
		...
		strbuf_add(&dir->internal.basebuf, base + current,
			   stk->baselen - current);
				  ...
				  const char *pathname, int len)
{
	...
	ALLOC_GROW(dir->ignored, dir->ignored_nr + 1,
		   dir->internal.ignored_alloc);
		...
		const struct pathspec *pathspec)
{
	...
	if ((dir->flags & DIR_SKIP_NESTED_GIT) ||
		 ...
		 * 2) `dirname` is not an immediate parent of
		 * `the_repository->gitdir`, which could occur if the git_dir or
		 * worktree location was manually configured by the user; see
		 * t2205 testcases 1-3 for examples where this matters
		 */
		int nested_repo;
		struct strbuf sb = STRBUF_INIT;
			 ...
			 struct index_state *istate)
{
	...
	if (dir->flags != dir->untracked->dir_flags) {
		/*
		 * If the untracked structure we received does not have the same
		 * flags as configured, then we need to reset / create a new
		 * "untracked" structure to match the new config.
		 *
		 * Keeping the saved and used untracked cache consistent with
		 * the configuration provides an opportunity for frequent users
		 * of "git status -uall" to leverage the untracked cache by
		 * aligning their configuration - setting
		 * "status.showuntrackedfiles" to "all" or "normal" as
		 * appropriate.
		 *
		 * Previously using -uall (or setting
		 * "status.showuntrackedfiles" to "all") was incompatible with
		 * untracked cache and *consistently* caused surprisingly bad
		 * performance (with fscache and fsmonitor enabled) on Windows.
		 *
		 * IMPROVEMENT OPPORTUNITY: If we reworked the untracked cache
		 * storage to not be as bound up with the desired output in a
		 * given run, and instead iterated through and stored enough
		 * information to correctly serve both "modes", then users could
		 * get peak performance with or without '-uall' regardless of
		 * their "status.showuntrackedfiles" config.
		 */
		if (dir->untracked->dir_flags !=
			...
			/*
			 * Current untracked cache data is consistent with
			 * config, but not usable in this request/run; just
			 * bypass untracked cache.
			 */
			return NULL;
		}
	}
	...
	if (!oideq(&dir->internal.ss_excludes_file.oid,
		...
		invalidate_gitignore(dir->untracked, root);
		dir->untracked->ss_excludes_file =
				      ...
				      int path_len)
{
	...
	trace2_data_intmax("read_directory", repo, "directories-visited",
			   dir->internal.visited_directories);
	trace2_data_intmax("read_directory", repo, "paths-visited",
			   dir->internal.visited_paths);

	if (!dir->untracked)
		return;
	trace2_data_intmax("read_directory", repo, "node-creation",
			   dir->untracked->dir_created);
	trace2_data_intmax("read_directory", repo, "gitignore-invalidation",
			   dir->untracked->gitignore_invalidated);
	trace2_data_intmax("read_directory", repo, "directory-invalidation",
			   dir->untracked->dir_invalidated);
	trace2_data_intmax("read_directory", repo, "opendir",
			   dir->untracked->dir_opened);
}
		   ...
		   const char *path, int len, const struct pathspec *pathspec)
{
	...
	if (dir->untracked) {
		...
		if (force_untracked_cache < 0)
			force_untracked_cache =
				(istate->repo->settings.core_untracked_cache ==
...
static int remove_dir_recurse(struct strbuf *path, int flag, int *kept_up)
{
	...
	if (!ret && !keep_toplevel && !kept_down) {
		if (!purge_original_cwd && startup_info->original_cwd &&
	...
	void setup_standard_excludes(struct dir_struct *dir)
{
	...
	if (excludes_file && !access_or_warn(excludes_file, R_OK, 0))
		add_patterns_from_file_1(
			dir, excludes_file,
			dir->untracked ? &dir->internal.ss_excludes_file :
					 NULL);
