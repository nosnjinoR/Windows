...
static void xml_start_tag(void *userData, const char *name,
...
static void one_remote_ref(const char *refname)
{
	...
	if (repo->can_update_info_refs &&
	    !repo_has_object_file(the_repository, &ref->old_oid)) {
...
int cmd_main(int argc, const char **argv)
{
	...
	for (ref = remote_refs; ref; ref = ref->next) {
		...
		if (oideq(&ref->old_oid, &ref->peer_ref->new_oid)) {
			if (push_verbosely)
				/* stable plumbing output; do not modify or
				 * localize */
		...
		if (!force_all && !is_null_oid(&ref->old_oid) && !ref->force) {
			if (!repo_has_object_file(the_repository,
						  &ref->old_oid) ||
