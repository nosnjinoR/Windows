...
static int insert_oid_pair(kh_oid_map_t *map, const struct object_id *key,
...
static int load_one_loose_object_map(struct repository *repo,
				     struct object_directory *dir)
{
	...
	insert_loose_map(dir, repo->hash_algo->empty_tree,
			 repo->compat_hash_algo->empty_tree);
	insert_loose_map(dir, repo->hash_algo->empty_blob,
			 repo->compat_hash_algo->empty_blob);
	insert_loose_map(dir, repo->hash_algo->null_oid,
	...
	while (!strbuf_getline_lf(&buf, fp)) {
		...
		if (parse_oid_hex_algop(buf.buf, &oid, &p, repo->hash_algo) ||
		    *p++ != ' ' ||
		    parse_oid_hex_algop(p, &compat_oid, &p,
...
int repo_write_loose_object_map(struct repository *repo)
{
	...
	strbuf_git_common_path(&path, repo, "objects/loose-object-idx");
	fd = hold_lock_file_for_update_timeout(&lock, path.buf,
					       LOCK_DIE_ON_ERROR, -1);
	iter = kh_begin(map);
	if (write_in_full(fd, loose_object_header,
			  strlen(loose_object_header)) < 0)
		goto errout;

	for (; iter != kh_end(map); iter++) {
		if (kh_exist(map, iter)) {
			if (oideq(&kh_key(map, iter),
				  the_hash_algo->empty_tree) ||
			    oideq(&kh_key(map, iter),
...
static int write_one_object(struct repository *repo,
			    const struct object_id *oid,
			    const struct object_id *compat_oid)
{
	struct lock_file lock;
	...
	if (fstat(fd, &st) < 0)
		goto errout;
	if (!st.st_size && write_in_full(fd, loose_object_header,
					 strlen(loose_object_header)) < 0)
		goto errout;
			      ...
			      struct object_id *dest)
{
	...
	for (dir = repo->objects->odb; dir; dir = dir->next) {
		...
		if (!loose_map)
			continue;
		map = (to == repo->compat_hash_algo) ? loose_map->to_compat :
