...
static void find_short_object_filename(struct disambiguate_state *ds)
{
	...
	for (odb = ds->repo->objects->odb; odb && !ds->ambiguous;
	     odb = odb->next)
		oidtree_each(odb_loose_cache(odb, &ds->bin_pfx), &ds->bin_pfx,
			     ds->len, match_prefix, ds);
}
...
static void find_short_packed_object(struct disambiguate_state *ds)
{
	...
	if (ds->bin_pfx.algo &&
	    (&hash_algos[ds->bin_pfx.algo] != ds->repo->hash_algo))
		return;
...
static int init_object_disambiguation(struct repository *r, const char *name,
				      int len, const struct git_hash_algo *algo,
				      struct disambiguate_state *ds)
{
	int i;
...
static int show_ambiguous_object(const struct object_id *oid, void *data)
{
	...
	assert(type == OBJ_TREE || type == OBJ_COMMIT || type == OBJ_BLOB ||
	       type == OBJ_TAG);
					 ...
					 unsigned flags)
{
	...
	if (!quietly && (status == SHORT_NAME_AMBIGUOUS)) {
		...
		repo_for_each_abbrev(r, ds.hex_pfx, algo, collect_ambiguous,
				     &collect);
...
int repo_for_each_abbrev(struct repository *r, const char *prefix,
			 const struct git_hash_algo *algo, each_abbrev_fn fn,
			 void *cb_data)
{
	struct oid_array collect = OID_ARRAY_INIT;
	struct disambiguate_state ds;
			 ...
			 struct object_id *oid, unsigned int flags)
{
	...
	if (!len && reflog_len)
		...
		refs_found =
			repo_dwim_ref(r, "HEAD", 4, oid, &real_ref, !fatal);
	else if (reflog_len)
...
static int reject_tree_in_index(struct repository *repo, int only_to_die,
				const struct cache_entry *ce, int stage,
				const char *prefix, const char *cp)
{
	if (!S_ISSPARSEDIR(ce->ce_mode))
		return 0;
	if (only_to_die)
