 ...
 * for speed.
 */
size_t repo_approximate_object_count(struct repository *r);
...
void *unpack_entry(struct repository *r, struct packed_git *, off_t, enum object_type *, size_t *);
size_t unpack_object_header_buffer(const unsigned char *buf, unsigned long len, enum object_type *type, size_t *sizep);
size_tg get_size_from_delta(struct packed_git *, struct pack_window **, off_t);
