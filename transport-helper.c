...
static struct child_process *get_helper(struct transport *transport)
{
	...
	if (have_git_dir())
		strvec_pushf(&helper->env, "%s=%s", GIT_DIR_ENVIRONMENT,
			     get_git_dir());
...
static void check_helper_status(struct helper_data *data)
{
	...
	if (pid > 0 && WIFEXITED(status))
		die("Remote helper '%s' died with %d", data->name,
		    WEXITSTATUS(status));
}
		      ...
		      struct ref **to_fetch)
{
	...
	if (process_connect(transport, 0))
		return transport->vtable->fetch_refs(transport, nr_heads,
						     to_fetch);
...
static struct transport_vtable vtable = { .set_option = set_helper_option,
