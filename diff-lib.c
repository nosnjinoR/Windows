...
void run_diff_files(struct rev_info *revs, unsigned int option)
{
	...
	for (i = 0; i < entries; i++) {
		 ...
		 */
		if (!ce_path_match(istate, ce, &revs->prune_data,
				   revs->ps_matched))
			continue;

		if (revs->diffopt.prefix &&
		    strncmp(ce->name, revs->diffopt.prefix,
			  ...
			  int match_missing)
{
	...
	if (new_file && S_ISSPARSEDIR(new_file->ce_mode)) {
		diff_tree_oid(NULL, &new_file->oid, new_file->name,
			      &revs->diffopt);
		return;
	}
			 ...
			 int report_missing, int cached, int match_missing)
{
	...
	if (S_ISSPARSEDIR(new_entry->ce_mode)) {
		diff_tree_oid(&old_entry->oid, &new_entry->oid, new_entry->name,
			      &revs->diffopt);
		return 0;
	}
			   ...
			   const struct cache_entry *tree)
{
	...
	if (!idx) {
		if (S_ISSPARSEDIR(tree->ce_mode)) {
			diff_tree_oid(&tree->oid, NULL, tree->name,
				      &revs->diffopt);
			return;
		}
...
void diff_get_merge_base(const struct rev_info *revs, struct object_id *mb)
{
	...
	if (repo_get_merge_bases(the_repository, mb_child[0], mb_child[1],
				 &merge_bases) < 0)
		exit(128);
	if (!merge_bases)
		       ...
		       int ita_invisible_in_index)
{
	...
	if (flags) {
		...
		if (flags->override_submodule_config)
			rev.diffopt.flags.ignore_submodules =
...
static struct strbuf *idiff_prefix_cb(struct diff_options *opt UNUSED,
				      void *data)
{
	return data;
}
