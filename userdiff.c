...
static struct userdiff_driver builtin_drivers[] = {
	...
	PATTERNS(
		"csharp",
		/*
		 * Jump over reserved keywords which are illegal method names,
		 * but which can be followed by parentheses without special
		 * characters in between, making them look like methods.
		 */
		 ...
		 * combination of keywords/type/name) before the opening
		 * parenthesis, and without final unexpected characters,
		 * normally only used in ordinary statements.
		 */
		...
		/*
		 * Properties:
		 * As with methods, expect a minimum of two groups. But, more
		 * trivial than methods, the vast majority of properties long
		 * enough to be worth showing a chunk header for don't include
		 * "=:;,()" on the line they are defined, since they don't have
		 * a parameter list.
		 */
		"^[ \t]*("
		"([][[:alnum:]@_.](<[][[:alnum:]@_, \t<>]+>)?)+"
